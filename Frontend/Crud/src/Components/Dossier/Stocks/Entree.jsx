...
function Entree() {
  const newUrl = window.location.href.replace('/entree', '')
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(`http://localhost:5000/api/enteteStock/getAllEntreeEnteteStockByDossier/${dossierId}`, {
          headers: {
  ...
  const filteredItems = Mvts.filter(item => {
    return (
      ...
      item.fournisseur?.nom.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.depot?.Libelle.toLowerCase().includes(searchQuery.toLowerCase())
    );
  });
  ...
  return (
    <div>
      <br />
      <div >
        <a href={window.location.href + '/ajouterEntree'}>
          <CButton color="primary" className='btnAjout'>Ajouter une Entrée</CButton>
        </a>
