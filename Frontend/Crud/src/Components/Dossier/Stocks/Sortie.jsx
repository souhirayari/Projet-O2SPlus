...
function Entree() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(`http://localhost:5000/api/enteteStock/getAllSortieEnteteStockByDossier/${dossierId}`, {
          headers: {
  ...
  return (
    <div>
      <br />
      <div >
        <a href={window.location.href + '/ajouterSortie'}>
          <CButton color="primary" className='btnAjout'>Ajouter une Sortie</CButton>
        </a>
